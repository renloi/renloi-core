...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of renloi is "renloi". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Renloi", keep that.
	if name == "renloi" || name == "renloi-testnet" {
		name = "Renloi"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "renloi" instances.
var isOldRenloiResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by renloi 1.4 are used if they exist.
	if warn, isOld := isOldRenloiResource[path]; isOld {
		oldpath := ""
		if c.name() == "renloi" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldRenloiResourceWarning, "Using deprecated resource file %s, please move this file to the 'renloi' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
