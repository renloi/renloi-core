...
1. initialize the renloi node with the `genesis.json` file contained in the `testdata` directory
2. import the `halfchain.rlp` file in the `testdata` directory
3. run renloi with the following flags:
```
renloi --datadir <datadir> --nodiscover --nat=none --networkid 19763 --verbosity 5
```

Then, run the following command, replacing `<enode>` with the enode of the renloi node:
 ```
...
To run the eth66 protocol test suite, initialize a renloi node as described above and run the following command,
replacing `<enode>` with the enode of the renloi node:
