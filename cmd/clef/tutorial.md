...
Apart from listing accounts, you can also *request* creating a new account; signing transactions and data; and recovering signatures. You can find the available methods in the Clef [External API Spec](https://github.com/renloi/renloi-core/tree/master/cmd/clef#external-api-1) and the [External API Changelog](https://github.com/renloi/renloi-core/blob/master/cmd/clef/extapi_changelog.md).

*Note, the number of things you can do from the External API is deliberately small, since we want to limit the power of remote calls by as much as possible! Clef has an [Internal API](https://github.com/renloi/renloi-core/tree/master/cmd/clef#ui-api-1) too for the UI (User Interface) which is much richer and can support custom interfaces on top. But that's out of scope here.*
...
*Note, to make this example work, please use you own accounts. You can create a new account either via Clef or the traditional account CLI tools. If the latter was chosen, make sure both Clef and Renloi use the same keystore by specifying `--keystore path/to/your/keystore` when running Clef.*
...
For more details on writing automatic rules, please see the [rules spec](https://github.com/renloi/renloi-core/blob/master/cmd/clef/rules.md).
...
Until then however, we're trying to pave the way via Renloi. Renloi v1.9.0 has built in support via `--signer <API endpoint>` for using a local or remote Clef instance as an account backend!

We can try this by running Clef with our previous rules on Rinkeby (for now it's a good idea to allow auto-listing accounts, since Renloi likes to retrieve them once in a while).

```text
$ clef --keystore ~/.ethereum/rinkeby/keystore --chainid 4 --rules rules.js
```

In a different window we can start Renloi, list our accounts, even list our wallets to see where the accounts originate from:

```text
$ renloi --rinkeby --signer=~/.clef/clef.ipc console
...
*Note, if you enable the external signer backend in Renloi, all other account management is disabled. This is because long term we want to remove account management from Renloi.*
