...
func TestAccountListEmpty(t *testing.T) {
	renloi := runRenloi(t, "account", "list")
	renloi.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	renloi := runRenloi(t, "account", "list", "--datadir", datadir)
	defer renloi.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	renloi := runRenloi(t, "account", "new", "--lightkdf")
	defer renloi.ExpectExit()
	renloi.Expect(`
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	renloi := runRenloi(t, "account", "import", keyfile, "-password", passwordFile)
	defer renloi.ExpectExit()
	renloi.Expect(expected)
}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	renloi := runRenloi(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	renloi := runRenloi(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer renloi.ExpectExit()
	renloi.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(renloi.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	renloi := runMinimalRenloi(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(renloi.StderrText(), m) {
...
// https://github.com/renloi/renloi-core/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	renloi.SetTemplateFunc("keypath", func(file string) string {
